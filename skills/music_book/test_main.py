"""Tests for music-book skill. stdlib unittest, golden-output pattern."""
...
def _run_skill(fixture_dir=None, fmt="json", extra_args=None):
    """Run music-book as a subprocess, return parsed result."""
    cmd = [sys.executable, "-m", "skills.music_book", "--format", fmt]
    if fixture_dir:
...
class TestHappyPath(unittest.TestCase):
    def test_happy_path_golden(self):
        ...
        run = _run_skill(
            ...
            extra_args=["--artist", "The Wanderers", "--date", "2026-05-02"],
        )
...
class TestWarnCase(unittest.TestCase):
    def test_date_conflict_golden(self):
        """Run against warn_case fixtures, verify date conflict finding."""
        run = _run_skill(
            str(FIXTURES / "warn_case"),
            extra_args=["--artist", "Echo Valley", "--date", "2026-04-18"],
        )
    ...
    def test_artist_not_found(self):
        """Search for nonexistent artist returns warn with ARTIST_NOT_FOUND."""
        run = _run_skill(
            str(FIXTURES / "happy_path"),
            extra_args=["--artist", "Nonexistent Band", "--date", "2026-05-01"],
        )
        self.assertEqual(run["returncode"], 1)
        actual = json.loads(run["stdout"])
        self.assertEqual(actual["status"], "warn")
        codes = [f["code"] for f in actual["findings"]]
        self.assertIn("ARTIST_NOT_FOUND", codes)

    def test_case_insensitive_match(self):
        """Artist lookup should be case-insensitive."""
        run = _run_skill(
            str(FIXTURES / "happy_path"),
            extra_args=["--artist", "echo valley", "--date", "2026-05-10"],
        )
        self.assertEqual(run["returncode"], 0, "stderr: " + run["stderr"])
        actual = json.loads(run["stdout"])
        self.assertEqual(actual["data"]["artist_info"]["name"], "Echo Valley")

    def test_phone_method(self):
        """Phone method generates a phone script."""
        run = _run_skill(
            str(FIXTURES / "happy_path"),
            extra_args=["--artist", "Echo Valley", "--date", "2026-05-10",
                         "--method", "phone"],
        ...
        self.assertEqual(run["returncode"], 0, "stderr: " + run["stderr"])
        actual = json.loads(run["stdout"])
        self.assertIn("PHONE SCRIPT", actual["data"]["message"])
