"""Tests for vendor-order skill. stdlib unittest, golden-output pattern."""
...
def _run_skill(fmt="json", extra_args=None):
    """Run vendor-order as a subprocess, return parsed result."""
    cmd = [sys.executable, "-m", "skills.vendor_order", "--format", fmt]
    if extra_args:
...
class TestHappyPath(unittest.TestCase):
    def test_happy_path_golden(self):
        """Vendor with orders -> order text generated."""
...
class TestWarnCase(unittest.TestCase):
    def test_warn_case_golden(self):
        """Filter for specific vendor -> draft generated."""
